...
////////////////////////////////////////////////////////
class AckermanModelInputs: public glc::Inputs{
public:
  //uniformly spaced points on a circle
  AckermanModelInputs(int num_steering_angles){
    //Make all pairs (forward_speed,steering_angle)
    glc::vctr car_speeds({-1.0,1.0});//Pure path planning
    glc::vctr steering_angles = glc::linearSpace(-0.25*M_PI,0.25*M_PI,num_steering_angles);
...
////////////////////////////////////////////////////////
class RectangularGoal3D: public glc::GoalRegion{
  double x_dim,y_dim,z_dim;
  glc::vctr center;
  int resolution;
public:
  RectangularGoal3D(const int& _state_dim, 
...
public:
  SingleIntegrator(const double& _max_time_step): SymplecticEuler(_max_time_step) {}
