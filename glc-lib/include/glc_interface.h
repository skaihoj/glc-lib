...
//     virtual bool inGoal(const vctr& state)=0;//TODO phase out
  };
...
//       std::cout << "step " << integration_step << std::endl;
//       Spline* ptr  = new Spline(integration_step,t0,x0.size(),4);
//       std::cout << "test t0 " << ptr->initialTime()  << std::endl; 
      solution = splinePtr(new Spline(integration_step,t0,x0.size(),4));
//       std::cout << "ref t0: " << t0 << std::endl;
//       std::cout << "solution t0: " << solution->initialTime() << std::endl;;
      ...
      sim_counter++;
//       if(solution->initialTime() != t0){std::cout << " Something fishy here " << std::endl;}
      return;
    }
  };
