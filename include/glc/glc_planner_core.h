...
#include <glc/glc_numerical_integration.h>
#include <glc/glc_state_equivalence_class.h>
#include <glc/glc_node.h>
#include <glc/glc_parameters.h>
#include <glc/glc_logging.h>
...
namespace glc{  
  ...
  class Planner{
//     static const std::shared_ptr<Node> INF_COST_NODE(new Node(0, -1, DBL_MAX, DBL_MAX,std::valarray<double>(0),0,nullptr));
    
  public:
    //Interface
    std::shared_ptr<Node> best;// = INF_COST_NODE;
    ...
    void add_child(std::shared_ptr<Node> parent, std::shared_ptr<Node> child);//TODO move to node class
    ...
    Planner(Obstacles* _obs, 
               ...
               h(_h){
                 ...
                 best = std::shared_ptr<Node>(new Node(0, -1, DBL_MAX, DBL_MAX,std::valarray<double>(0),0,nullptr));
  ...
  void Planner::add_child(std::shared_ptr<Node> parent, std::shared_ptr<Node> child){
    child->parent = parent;
...
//       std::vector< std::vector< std::valarray<double> > > coef(traj_segment->coefficient_array);//USED?
    }
